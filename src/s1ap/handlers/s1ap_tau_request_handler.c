...
int tau_request_handler(struct proto_IE *s1_tau_req_ies, int enb_fd)
{
    ...
    for (int i = 0; i < s1_tau_req_ies->no_of_IEs; i++)
    {
        switch (s1_tau_req_ies->data[i].IE_type)
        {
            ...
            case S1AP_IE_TAI:
            {
                memcpy(&(req.msg_data.tauReq_Q_msg_m.tai),
                        &(s1_tau_req_ies->data[i].val.tai), sizeof(struct TAI));
            }
            break;
            case S1AP_IE_UTRAN_CGI:
            {
                memcpy(&(req.msg_data.tauReq_Q_msg_m.eUtran_cgi),
                        &(s1_tau_req_ies->data[i].val.utran_cgi),
                        sizeof(struct CGI));
            }
            break;
